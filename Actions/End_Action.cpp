...
End_Action::End_Action(ApplicationManager* pAppManager) :Action(pAppManager)
{
}

void End_Action::ReadActionParameters()
{
...
void End_Action::Execute()
{
	ReadActionParameters();
	if (pManager->GetStatCount() == 0) return;		//first statement must be Start statement
	//Calculating ceneter of end statement block
	Point Center;

	Center.x = Position.x;
	Center.y = Position.y;

	End_Stat* pEnd = new End_Stat(Center);
